...
#updated version of original script to include attempts at using geopy geolocation library to identify the relevant country from the original data
...
#set up geopy with GeoNames as it's the free service
geopy_api_rate_limit = 1 #wait one second between requests
geopy_api_usage_limit = 2000 #maximum number of API calls per day
geolocator = geopy.geocoders.GoogleV3(api_key="AIzaSyBs4u-0We3vNK7v4RwGTKnIV5LIcxU0EDE")
			...
			try:
				...
				if (got_results):
					if 'country' not in output:
						#use geopy to work out the country
						if (count < geopy_api_usage_limit):
							# turbotlib.log(str(count) +  	" " + category + " " + company_name)
							count += 1
							address_input = output['address']
							location = geolocator.geocode(address_input, True)
							location_country = location.address.split(",")[-1].strip()
							print(address_input + " --- " + location_country)
							time.sleep(geopy_api_rate_limit)
